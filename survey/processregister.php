...
function mysqli_result($res,$row=0,$col=0){ 
	...
	require_once("../database.php");
		...
		{
			$sql = "SELECT COUNT(*) FROM surveyusers WHERE name = '" . mysqli_real_escape_string($result,$name) . "'";
			$ref = $result->query($sql);
			$result1 = mysqli_result($ref,0);
			if($result1 == 0)
			{
					$sql = "INSERT INTO surveyusers (name, password,  mob, college, email, role) VALUES ('" . mysqli_real_escape_string($result,$name) . "','" . mysqli_real_escape_string($result,$password) . "','". $phone . "','" . mysqli_real_escape_string($result,$college) .  "','" . mysqli_real_escape_string($result,$email) . "','1')";
					$ref = $result->query($sql);
