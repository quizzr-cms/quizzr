...
<body>
	...
	global $result;
	$sql = "SELECT name,level,college FROM treasusers WHERE role!=-1 ORDER BY level DESC";
	...
	$sql = "SELECT name,level,college FROM treasusers WHERE role!=-1 ORDER BY level DESC, passtime ASC LIMIT $offset,$per";
	$ref = $result->query($sql);
	$rank=$offset+1;
	...
	{	if($page==1){
		echo "<tr class=\"add classname here if any\" id=\"rank".$rank."\"><td>".$rank."</td><td>".$row["name"]."</td><td>".$row["college"]."</td><td>".$row["level"]."</td></tr>";
		$rank++;
		}
		else{
		echo "<tr class=\"add classname here if any\"><td>".$rank."</td><td>".$row["name"]."</td><td>".$row["college"]."</td><td>".$row["level"]."</td></tr>";
		$rank++;
		}
	} 
	?></table>
